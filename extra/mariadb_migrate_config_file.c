...
static const char *mariadb_replication_compatibility_comment=
  "# The following options enables MySQL to replicate to MariaDB";
static const char *mysql_replication_compatibility_comment=
  "# The following options enables MariaDB to replicate to MySQL";
...
void error_unknown_option(struct upgrade_ctx *ctx,
                          const char *option, const char *value,
                          const char *org_line,
                          const char *file_name, uint line)
{
  const char *renamed;
  ...
  else
  {
    fprintf(stdout, "In %s at line %u: Invalid option %s\n", file_name, line,
            option);
    if (!strncmp(option, STRING_WITH_LEN("audit_log_")) &&
        !audit_plugin_warning++ && !opt_silent)
      fprintf(stdout, "Please replace audit_log with the server_audit "
              "plugin\n");
  }
  ctx->failed= 1;
  global_update_count++;
                                         ...
                                         int recursion_level)
{
  ...
  my_bool file_valid= TRUE, group_is_mariadbd= FALSE;
  uint line=0, option_start;
  enum { NONE, PARSE, SKIP } found_group= NONE;
  my_bool found_skip_slave_start= FALSE, generator_inited= 0;
  my_bool found_innodb_log_files_in_group= 0, found_innodb_log_file_size= 0;
  my_bool found_character_set_server= 0;
  my_bool skip_prefix= 0;
  uint value_innodb_log_files_in_group= 0;
  ulonglong value_innodb_log_file_size= 0;
  char suffix_innodb_log_file_size;
  size_t i;
  MY_DIR *search_dir;
  ...
  while (mysql_file_fgets(buff, sizeof(buff) - 3, fp))
  {
    ...
    if (*ptr == '#' || *ptr == ';' || !*ptr)
    {
      if (!is_prefix(mariadb_replication_compatibility_comment, ptr))
        opt_add_mariadb_replication_compatibility= 0;   /* File is already updated */
      if (!is_prefix(mysql_replication_compatibility_comment, ptr))
        opt_add_mysql_replication_compatibility= 0;     /* File is already updated */
	...
	goto err;
      ...
      group_is_mariadbd= !strncmp(curr_gr, "mariadbd", 8);
	...
	value_end--;
      ...
      *ptr++= 0;
      ...
      if (!mariadbd_option_exists(option))
      {
        ...
        if (!opt_print && opt_edit_mode == EDIT_MODE_NONE)
        {
          error_unknown_option(ctx, conv_option, value, buff, name, line);
          continue;
        }
      }
	...
	if (*value == '\\' && value != value_end-1)
	{
      ...
      *ptr=0;
      ...
      else if (check_plugin_option(ctx, option, option_value_start, name, line))
      {
        line_valid= FALSE;
        file_valid= FALSE;
      }
             ...
             !(skip_prefix && mariadbd_option_exists(conv_option)))
    {
      ...
      {
        error_unknown_option(ctx, conv_option, NullS, buff, name, line);
        continue;
      }
    }
    else if (check_plugin_option(ctx, option, option_value_start, name, line))
  ...
  {
    file_valid= FALSE;
    /* We do this line by line to get correct row numbers for error reporting */
  ...
  if (opt_add_mysql_replication_compatibility)
  {
    ...
    if (!found_character_set_server)
    {
      ...
      if (!opt_silent)
        fprintf(stdout, "Note: Added MySQL default character sets and collation to "
                "[mariadbd] to be able to replicate to MySQL\n");
    }
  }
...
static struct my_option my_long_options[] =
{
  ...
  {"add-mysql-replication-compatibility",
   OPT_ADD_MYSQL_REPLICATION_COMPATIBILITY,
   "Add options to [mariadbd to enable MariaDB to replicate to MySQL",
   &opt_add_mysql_replication_compatibility,
   &opt_add_mysql_replication_compatibility,
   0, GET_BOOL, NO_ARG, FALSE, 0, 0, 0, 0, 0},
  {"backup", OPT_BACKUP,
  ...
  {"copy-mysqld-groups", OPT_COPY_MYSQLD_GROUPS,
   "Copy valid MariaDB options from [mysqld-...] groups to [mariadbd] group. "
   "On by default",
  ...
  {"fix-all", OPT_FIX_ALL,
   ...
   "--add-mysql-replication-compatibility "
   "--add-mariadb-replication-compatibility --edit=last",
...
static void usage()
{
  ...
  puts("");
  puts("- Check the how the new configuration files would look like:");
  ...
  puts("Examples:");
  pe("", "Go through all my.cnf files and prints not supported options. "
     "The groups checked are [server], [mysqld], and [mysqld-###]");
  pe("--from=mariadb",
     "Go through all my.cnf files and prints options not supported by "
     "the current MariaDB version. All MariaDB groups read by mariadbd are "
     "checked");
  pe("~/.my.cnf", "Print not supported options in ~/.my.cnf");
  pe("--defaults-file=/etc/my.cnf --verify=1",
  ...
  pe("--print --edit=last",
     "Print modified version of all my.cnf files where supported options from "
